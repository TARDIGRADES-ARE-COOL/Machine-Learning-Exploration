...
def compute_gradient(X, y, w, b, *argv):
    """
    Computes the gradient for logistic regression using vectorized operations.
    ...
    Returns:
      ...
      dj_dw : (ndarray Shape (n,)) gradient w.r.t. weights
    """
    m = X.shape[0]
...
def predict(X, w, b): 
    ...
    for i in range(m):   
        ...
        # Apply the threshold
        p[i] = p[i] = 1 if f_wb >= 0.5 else 0   # <-- threshold at 0.5
